...
```{r}
  ...
  my_df <- data.frame(name = murders$state, population = ranks)
  my_df
```
...
```{r}
  ...
  my_df <- data.frame(name = murders$state, population = murders$population[ind])
  my_df
```
...
```{r}
  ...
  murders$state[murder_rate < murder_mean]
```
...
```{r}
  M_ <- c("MA", "ME", "MI", "MO", "MU")
  M_[which(c("MA", "ME", "MI", "MO", "MU") %in% murders$abb == TRUE)]
```
...
```{r}
  murders$populations <- with(murders, population)
  boxplot(populations~region, data = murders)
```
